...
def create_box_plot(df, event_filter, y_column, title, color_column=None, overlay_performance=False, 
    ...
    if overlay_performance and performance_df is not None and not performance_df.empty:
        ...
        if y_column == 'time_value' and perf_data_column in perf_filtered.columns:
            ...
            # Ranking points overlay - show both Ranking_Score and PfSc
            overlay_added = False
            
            # Add Ranking_Score overlay (white border)
            if 'Ranking_Score' in perf_filtered.columns and show_ranking_score:
                ...
                if not ranking_valid.empty:
                    # Create hover data for ranking scores
                    ranking_hover_data = []
                    ...
                    fig.add_trace(go.Scatter(
                        ...
                        x=[1.1] * len(ranking_valid),  # Slight offset
                        mode='markers',
                    ...
                    overlay_added = True
                    st.info(f"📊 Added {len(ranking_valid)} current ranking scores (blue circles with white border)")
            
            # Add PfSc overlay (Performance Scores)
            if 'PfSc' in perf_filtered.columns and show_pfsc_scores:
                pfsc_valid = perf_filtered.dropna(subset=['PfSc'])
                if not pfsc_valid.empty:
                    # Create hover data for performance scores
                    pfsc_hover_data = []
                    ...
                    fig.add_trace(go.Scatter(
                        y=pfsc_valid['PfSc'],
                        x=[1.3] * len(pfsc_valid),  # Further offset
                        mode='markers',
