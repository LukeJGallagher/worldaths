    ...
    try:
        ...
        for file in csv_files:
            try:
                df = pd.read_csv(file)
                all_data.append(df)
                st.sidebar.success(f"✅ Loaded {os.path.basename(file)}")
            except Exception as e:
                st.sidebar.error(f"❌ Error loading {os.path.basename(file)}: {e}")
        
        if all_data:
            combined_df = pd.concat(all_data, ignore_index=True)
            st.sidebar.success(f"📊 Total records: {len(combined_df)}")
            return combined_df
        else:
            return pd.DataFrame()
...
def create_box_plot(df, event_filter, y_column, title, color_column=None, overlay_performance=False, performance_df=None):
    ...
    if overlay_performance and performance_df is not None and not performance_df.empty and y_column == 'time_value':
        # Filter performance data for the same event
        if event_filter != "All Events":
            perf_filtered = performance_df[performance_df['Event_Name'].str.contains(event_filter.replace("'", ""), case=False, na=False)]
        else:
            perf_filtered = performance_df
        
        if not perf_filtered.empty and 'Result' in perf_filtered.columns:
            # Parse performance results
            perf_filtered = perf_filtered.copy()
            perf_filtered['parsed_result'] = perf_filtered['Result'].apply(parse_performance_result)
            perf_valid = perf_filtered.dropna(subset=['parsed_result'])
            
            if not perf_valid.empty:
                # Create custom hover data for performance points
                perf_hover_data = []
                ...
                # Add performance data as scatter points
                fig.add_trace(go.Scatter(
                    y=perf_valid['parsed_result'],
                    x=[1.2] * len(perf_valid),  # Offset to the right
                    mode='markers',
                    name='Competition Performances',
                    marker=dict(
                        color='red',
                        size=8,
                        symbol='diamond',
                    ...
                    hovertemplate="<b>%{text}</b><br>" +
                                ...
                                "<extra>Competition History</extra>"
                ))
                
                st.info(f"📊 Added {len(perf_valid)} competition performances to the visualization")
...
def main():
    ...
    if 'selected_events_state' not in st.session_state:
        st.session_state.selected_events_state = events[:10] if len(events) > 10 else events  # Start with first 10
